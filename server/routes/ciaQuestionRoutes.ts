...
const { processCollegeCIAWorkbook } = require("../utils/ciaCollegeWorkbook");

const router = express.Router();
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024 } });
const collegeImportDirectory = path.join(os.tmpdir(), "attainment-cia-imports");
fs.mkdirSync(collegeImportDirectory, { recursive: true });
const collegeUpload = multer({
  dest: collegeImportDirectory,
  limits: { fileSize: 100 * 1024 * 1024 },
  fileFilter: (_req, file, done) => done(null, /\.(xlsx|xlsm)$/i.test(file.originalname)),
});
...
async function migrateLegacyDepartmentImports() {
...
let legacyDepartmentImportCheck = null;

async function ensureLegacyDepartmentImports() {
  // This endpoint is refreshed frequently by the admin page. Avoid loading
  // every CIA student array once department summary records already exist.
  if (!legacyDepartmentImportCheck) {
    legacyDepartmentImportCheck = (async () => {
      const existingImportCount = await CIADepartmentImport.countDocuments({});
      if (existingImportCount > 0) return;
      await migrateLegacyDepartmentImports();
    })().catch((error) => {
      // A temporary MongoDB failure must not permanently disable the check.
      legacyDepartmentImportCheck = null;
      throw error;
    });
  }
  return legacyDepartmentImportCheck;
}
...
async function finishCollegeWorkbookImport(importId, filePath, academicYear, sourceFileName, importedBy) {
  try {
    const workbookImport = await CIAWorkbookImport.findById(importId);
    const result = await processCollegeCIAWorkbook({ filePath, workbookImport, academicYear, sourceFileName });
    await AcademicYear.findOneAndUpdate({ year: academicYear }, { $set: { isActive: true } }, { upsert: true, new: true });

    const departmentImports = [];
    for (const department of result.departments) {
      // The successfully parsed college master is the source of truth for this
      // department/year. Remove only older datasets after the new copy exists.
      const staleFilter = {
        departmentKey: department.key,
        academicYear,
        $or: [
          { workbookImport: { $ne: importId } },
          { importedAt: { $lt: workbookImport.startedAt } },
        ],
      };
      await Promise.all([
        CIAQuestionSet.deleteMany(staleFilter),
        CIAActivitySet.deleteMany(staleFilter),
      ]);
      const [questionDocs, activityDocs] = await Promise.all([
        CIAQuestionSet.find({ workbookImport: importId, departmentKey: department.key, academicYear }).lean(),
        CIAActivitySet.find({ workbookImport: importId, departmentKey: department.key, academicYear }).lean(),
      ]);
      const validation = validateDepartmentData(questionDocs, activityDocs);
      const duplicateRowCount = [...questionDocs, ...activityDocs].reduce((sum, doc) => sum + Number(doc.duplicateRowCount || 0), 0);
      const totalMismatchCount = [...questionDocs, ...activityDocs].reduce((sum, doc) => sum + Number(doc.totalMismatchCount || 0), 0);
      const unresolvedRowCount = questionDocs.reduce((sum, doc) => sum + Number(doc.unresolvedStudentCount || 0), 0);
      const mappingMissingCount = questionDocs.filter((doc) => doc.mappingStatus !== "COMPLETE").length;
      const now = new Date();
      const updated = await CIADepartmentImport.findOneAndUpdate(
        { departmentKey: department.key, academicYear },
        {
          $set: {
            ...
            academicYear,
            sourceFileName,
            sourceFileHash: workbookImport.sourceFileHash,
            workbookImport: importId,
            dataScope: "COLLEGE_MASTER",
            terms: result.terms,
          ...
          $push: {
            history: {
              ...
              at: now,
              version: department.version,
              note: `Imported from college master workbook ${sourceFileName}; department verification reset.`,
            },
          },
        },
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
      departmentImports.push(updated._id);
    }

    const hasSourceReview = Number(result.counts.mappingMissing || 0) > 0 || Number(result.counts.unresolvedRows || 0) > 0;
    await CIAWorkbookImport.findByIdAndUpdate(importId, {
      $set: {
        status: hasSourceReview ? "PARTIAL" : "SUCCESS",
        terms: result.terms,
        sheets: result.sheets,
        counts: result.counts,
        issues: result.issues,
        departmentImports,
        "progress.stage": hasSourceReview ? "Imported — source review required" : "Import completed",
    ...
    await CIAWorkbookImport.findByIdAndUpdate(importId, {
      $set: {
        status: "FAILED",
        "progress.stage": "Import failed",
        "progress.percent": 100,
        completedAt: new Date(),
      },
      $push: { issues: { severity: "critical", code: "IMPORT_FAILED", message: error.message || "Unknown import error" } },
    }).catch(() => {});
    console.error("College CIA master import failed", error);
  } finally {
    await fs.promises.unlink(filePath).catch(() => {});
  }
}

router.post("/import-college", collegeUpload.single("file"), async (req, res) => {
  if (!req.user.isAdmin) {
    if (req.file?.path) await fs.promises.unlink(req.file.path).catch(() => {});
    return res.status(403).json({ message: "Admin access only" });
  }
  if (!req.file) return res.status(400).json({ message: "College CIA Excel workbook is required (.xlsx or .xlsm)" });
  const academicYear = clean(req.body.academicYear);
  if (!/^20\d{2}-20\d{2}$/.test(academicYear)) {
    await fs.promises.unlink(req.file.path).catch(() => {});
    return res.status(400).json({ message: "Select a valid academic year such as 2025-2026" });
  }
  try {
    const sourceFileHash = await hashFile(req.file.path);
    const existing = await CIAWorkbookImport.findOne({ academicYear, sourceFileHash });
    if (existing?.status === "PROCESSING") {
      await fs.promises.unlink(req.file.path).catch(() => {});
      return res.status(409).json({ message: "This workbook is already being imported", importId: existing._id });
    }
    const workbookImport = existing || new CIAWorkbookImport({ academicYear, sourceFileHash });
    workbookImport.sourceFileName = req.file.originalname;
    workbookImport.sourceFileBytes = req.file.size;
    workbookImport.importedBy = req.user.staff_id || "admin";
    workbookImport.status = "PROCESSING";
    workbookImport.terms = [];
    workbookImport.sheets = [];
    workbookImport.counts = {};
    workbookImport.progress = { stage: "Queued for streamed validation", processed: 0, total: 16, percent: 0, currentSheet: "" };
    workbookImport.issues = [];
    workbookImport.departmentImports = [];
    workbookImport.startedAt = new Date();
    workbookImport.completedAt = null;
    await workbookImport.save();

    setImmediate(() => finishCollegeWorkbookImport(
      workbookImport._id,
      req.file.path,
      academicYear,
      req.file.originalname,
      req.user.staff_id || "admin"
    ));
    return res.status(202).json({
      message: "College CIA master workbook upload completed; streamed validation and MongoDB import started.",
      importId: workbookImport._id,
      status: workbookImport.status,
      fileSize: req.file.size,
    });
  } catch (error) {
    await fs.promises.unlink(req.file.path).catch(() => {});
    return res.status(500).json({ message: "Could not start college CIA workbook import", error: error.message });
  }
});

router.get("/admin/workbook-imports", async (req, res) => {
  if (!req.user.isAdmin) return res.status(403).json({ message: "Admin access only" });
  const imports = await CIAWorkbookImport.find({})
    .select("academicYear sourceFileName status counts progress startedAt completedAt createdAt updatedAt")
    .sort({ createdAt: -1 })
    .limit(25)
    .lean();
  res.json({ imports });
...
router.get("/admin/workbook-imports/:id", async (req, res) => {
  if (!req.user.isAdmin) return res.status(403).json({ message: "Admin access only" });
  const record = await CIAWorkbookImport.findById(req.params.id).lean();
  if (!record) return res.status(404).json({ message: "CIA workbook import not found" });
  res.json(record);
});
...
router.get("/admin/department-imports", async (req, res) => {
  ...
  try {
    await ensureLegacyDepartmentImports();
    const imports = await CIADepartmentImport.find({})
      .select("departmentName departmentKey academicYear sourceFileName terms version paperCount classCount studentCount questionSetCount activitySetCount questionRows activityRows inferredQuestionMaxCount inferredActivityMaxCount dataScope workbookImport duplicateRowCount totalMismatchCount unresolvedRowCount mappingMissingCount criticalCount warningCount status verifiedBy verifiedAt importedAt updatedAt")
      .sort({ academicYear: -1, departmentName: 1 })
      .lean();
    res.json({
      imports: imports.map((item) => ({
        ...
        workbookImport: item.workbookImport,
        duplicateRowCount: item.duplicateRowCount,
        totalMismatchCount: item.totalMismatchCount,
        unresolvedRowCount: item.unresolvedRowCount,
        mappingMissingCount: item.mappingMissingCount,
        criticalCount: item.criticalCount,
        warningCount: item.warningCount,
        status: item.status,
...
router.get("/admin/department-imports/:id", async (req, res) => {
  ...
  try {
    ...
    const activityMap = new Map<string, any>(activitySets.map((set) => [`${set.paperCodeKey}|${set.term}`, set]));
    ...
    for (const set of questionSets) {
      ...
      const info = {
        ...
        mappingStatus: set.mappingStatus || ((set.questions || []).every((q) => /^CO\d+$/i.test(clean(q.co))) ? "COMPLETE" : "MISSING"),
        duplicateRows: set.duplicateRowCount || 0,
        totalMismatches: set.totalMismatchCount || 0,
      };
...
router.patch("/admin/datasets/:id/questions", async (req, res) => {
  ...
  try {
    ...
    const updates = new Map<string, any>(req.body.questions.map((q) => [String(q.key), q]));
...
router.patch("/admin/activity-datasets/:id/components", async (req, res) => {
  ...
  try {
    ...
    const updates = new Map<string, any>(req.body.components.map((component) => [String(component.key).toUpperCase(), component]));
...
router.post("/:allocationId/activities/verify", async (req, res) => {
  ...
  const bySource = new Map<string, any>(summary.map((component) => [String(component.sourceKey || "").toUpperCase(), component]));
