...
// Admin can edit every allocation; allocated staff can enter/correct their CIA.
...
router.post("/:allocationId/bulk", async (req, res) => {
  ...
  for (const entry of entries) {
    const fixed = {};
    for (const [key, mark] of Object.entries(entry.componentMarks || {}) as [string, any][]) {
    ...
    if (Object.keys(fixed).length > 0) {
      ops.push({
        updateOne: {
          ...
          update: { $set: { componentMarks: fixed, calculationReady: true, source: req.user.isAdmin ? "admin" : "staff" } },
          upsert: true,
        },
      });
    } else {
  ...
  for (const row of rows) {
    ...
    if (Object.keys(componentMarks).length > 0) {
      await CIAMark.findOneAndUpdate(
        ...
        { $set: { componentMarks, calculationReady: true, source: req.user.isAdmin ? "admin" : "staff" } },
        { upsert: true }
      );
      updated++;
    }
