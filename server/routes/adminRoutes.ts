...
function departmentAccountDefinitions(department) {
  const baseCode = normalizeDepartmentCode(department.department_code);
  const aviation = (department.programs || []).filter((program) => /AVIATION/i.test(`${program.program_name || ""} ${program.main || ""}`));
  const split = aviation.map((program) => {
    const text = `${program.program_name || ""} ${program.main || ""}`;
    const kind = /BBA/i.test(text) ? "BBA" : /B\.?SC/i.test(text) ? "BSC" : "";
    if (!kind) return null;
    return {
      ...department,
      department_code: `${baseCode}${kind}`,
      source_department_code: baseCode,
      department_name: kind === "BSC" ? "Aviation (B.Sc.)" : "Aviation (BBA)",
      programs: [program],
    };
  }).filter(Boolean);
  return split.length >= 2 ? split : [{ ...department, source_department_code: baseCode }];
}
...
router.post("/department-accounts/sync", async (req, res) => {
  try {
    ...
    const definitions = departments.flatMap(departmentAccountDefinitions);
    for (const department of definitions) {
      ...
      if (!account) {
        ...
        account = new DepartmentAccount({
          ...
          sourceDepartmentCode: department.source_department_code || departmentCode,
          programmeIds: (department.programs || []).map((program) => program.program_id).filter(Boolean),
        ...
        account.sourceDepartmentCode = department.source_department_code || departmentCode;
        account.programmeIds = (department.programs || []).map((program) => program.program_id).filter(Boolean);
    ...
    res.json({ message: "Department accounts synchronized", received: definitions.length, created, updated, historicalTagged });
  } catch (error) {
...
router.get("/historical-attainment", async (req, res) => {
  ...
  const filter: any = { isLatest: req.query.includeVersions === "1" ? { $in: [true, false] } : true };
...
router.get("/attainment-records", async (req, res) => {
  ...
  const staffById = new Map<string, any>(staffDocs.map((s) => [s.staff_id, s]));
