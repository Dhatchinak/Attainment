...
async function accountBatchIds(account) {
  if (!(account.programmeIds || []).length && !(account.programmeAliases || []).length) return [];
  const aliasPatterns = (account.programmeAliases || []).map((alias) => new RegExp(escapeRegex(alias), "i"));
  return Batch.distinct("_id", { $or: [
    ...(account.programmeIds?.length ? [{ program_id: { $in: account.programmeIds } }] : []),
    ...(aliasPatterns.length ? [{ course: { $in: aliasPatterns } }] : []),
  ] });
}
...
router.get("/academic-years", async (req, res) => {
  const account = await accountFor(req);
  if (!account) return res.status(404).json({ message: "Department account is unavailable" });
  const staffIds = await Staff.distinct("staff_id", { department_code: account.sourceDepartmentCode || account.departmentCode });
  const batchIds = await accountBatchIds(account);
  const allocationFilter: any = { staff_id: { $in: staffIds }, isActive: true };
  if (batchIds.length) allocationFilter.batch = { $in: batchIds };
  const liveYearIds = await Allocation.distinct("academicYear", allocationFilter);
...
router.get("/records", async (req, res) => {
  ...
  const staffDocs = await Staff.find({ department_code: account.sourceDepartmentCode || account.departmentCode }).select("staff_id name salute").lean();
  ...
  const batchIds = await accountBatchIds(account);
  const liveFilter: any = { staff_id: { $in: staffIds }, academicYear: yearDoc?._id, isActive: true };
  if (batchIds.length) liveFilter.batch = { $in: batchIds };
  const allocations = yearDoc ? await Allocation.find(liveFilter).populate("batch").populate("academicYear").sort({ semester: 1, paperCode: 1 }) : [];
