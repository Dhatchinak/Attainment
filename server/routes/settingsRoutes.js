...
async function assertStaffOwnership(req, allocationId) {
  ...
  if (req.user.isAdmin) {
    return { error: "Threshold settings must be configured by the allocated staff member.", status: 403 };
  }
...
router.post("/:allocationId", async (req, res) => {
  ...
  if (target <= 0 || target > 100) {
    return res.status(400).json({ message: "Target must be between 1 and 100" });
  }
