...
router.post("/:allocationId/compute", async (req, res) => {
  ...
  if (!settings) {
    return res.status(400).json({ message: "Set the marks threshold and CIA/ESE weights before computing" });
  }
  ...
  if (questionWise) {
    ...
    const verificationByStage = new Map<string, any>(verifications.map((v) => [v.stage, v]));
...
router.patch("/:allocationId/remarks", async (req, res) => {
  ...
  const existingAttainment = await Attainment.findOne({ allocation: allocation._id }).lean();
  if (!existingAttainment) return res.status(400).json({ message: "Compute attainment before saving remarks" });
  const threshold = Number(existingAttainment.thresholdMarksPercent ?? 50);
  const outcomes = [...(existingAttainment.poAttainment || []), ...(existingAttainment.psoAttainment || [])];
  const belowKeys = new Set(outcomes.filter((row) => {
    const expected = Number(row.expected) || 0;
    const percentage = expected > 0 ? ((Number(row.value) || 0) / expected) * 100 : 100;
    return expected > 0 && percentage < threshold;
  }).map((row) => String(row.po || row.pso).toUpperCase()));
  ...
  for (const [key, value] of Object.entries(req.body?.outcomeRemarks || {})) {
    ...
    if (belowKeys.has(outcome)) outcomeRemarks[outcome] = text;
  }
...
router.post("/:allocationId/complete", async (req, res) => {
  ...
  const current = await Attainment.findOne({ allocation: allocation._id });
  if (!current) return res.status(400).json({ message: "Compute attainment at least once before marking this paper complete" });
  const threshold = Number(current.thresholdMarksPercent ?? 50);
  const missing = [...(current.poAttainment || []), ...(current.psoAttainment || [])].filter((row) => {
    const expected = Number(row.expected) || 0;
    if (expected <= 0) return false;
    const percentage = ((Number(row.value) || 0) / expected) * 100;
    const key = String(row.po || row.pso || "").toUpperCase();
    return percentage < threshold && !String(current.outcomeRemarks?.[key] || "").trim();
  }).map((row) => row.po || row.pso);
  if (missing.length) return res.status(400).json({ message: `Remarks are required for outcomes below ${threshold}%: ${missing.join(", ")}` });
