...
async function bestSourceForAllocation(candidates, allocation) {
  ...
  if (!batchId) return candidates[0];
  ...
  return candidates
    .map((source) => ({
      source,
      score: (source.students || []).reduce((count, student) => count + (allowed.has(normaliseRegNo(student.regNo)) ? 1 : 0), 0),
    }))
    .sort((a, b) => b.score - a.score)[0]?.source || candidates[0];
}
...
async function findQuestionSet(allocation, exam) {
  ...
  if (!source) {
    const candidates = await CIAQuestionSet.find({ paperCodeKey: key, exam, term, academicYear }).sort({ importedAt: -1 }).lean();
    source = await bestSourceForAllocation(candidates, allocation);
  }
  if (!source) {
    ...
    const legacyQuery: any = { paperCodeKey: key, exam, term, academicYear: "" };
...
async function findActivitySet(allocation) {
  ...
  if (!source) {
    const candidates = await CIAActivitySet.find({ paperCodeKey: key, term, academicYear }).sort({ importedAt: -1 }).lean();
    source = await bestSourceForAllocation(candidates, allocation);
  }
  if (!source) {
    const legacyQuery: any = { paperCodeKey: key, term, academicYear: "" };
    if (departmentKey) legacyQuery.departmentKey = { $in: [departmentKey, ""] };
