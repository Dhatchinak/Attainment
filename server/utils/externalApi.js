...
async function fetchStaffFromERP(staffId) {
  ...
  const referer = process.env.STAFF_API_REFERER; // e.g. http://10.240.151.162
...
async function fetchDepartmentsFromERP() {
  ...
  const referer = process.env.STAFF_API_REFERER; // same college ERP, same referer requirement
