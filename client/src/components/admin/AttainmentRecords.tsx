...
import HistoricalAttainmentArchive from "./HistoricalAttainmentArchive";

function departmentLabel(item) {
  const course = String(item.batch?.course || "");
  if (/AVIATION/i.test(course) && /B\.?SC/i.test(course)) return "Aviation (B.Sc.)";
  if (/AVIATION/i.test(course) && /BBA/i.test(course)) return "Aviation (BBA)";
  return item.staff.department_name || item.staff.department_code || "-";
}
...
export default function AttainmentRecords() {
  ...
  return (
    <div className="space-y-5">
      <div className="card-surface p-4 flex items-center justify-between gap-4 flex-wrap"><div><h2 className="font-display text-lg font-bold">All Attainment Records</h2><p className="text-sm text-gray-500">Current portal calculations and previous-system archive in one place.</p></div><div className="flex gap-2"><button onClick={() => setSource("current")} className={`btn ${source === "current" ? "btn-primary" : "btn-ghost"}`}>Current Portal Records</button><button onClick={() => setSource("historical")} className={`btn ${source === "historical" ? "btn-primary" : "btn-ghost"}`}>Previous System Records</button></div></div>
      {source === "historical" ? <HistoricalAttainmentArchive /> : <div className="card-surface p-5">
              ...
              {visible.map((item) => {
                ...
                return (
                  <tr key={item.allocation._id}>
                    ...
                    <td className="!text-left">{departmentLabel(item)}</td>
