...
function departmentLabel(item) {
  const text = `${item.department || ""} ${item.courseTitle || ""} ${item.courseCode || ""}`;
            ...
            <table className="pro-table">
              ...
              <tbody>
                {result.items.map((item) => <tr key={item._id}>
                  ...
                  <td className="!text-left"><strong>{departmentLabel(item)}</strong><div className="text-xs text-slate-500">Batch {item.batch} · {item.section === "NIL" ? "Aided (NIL)" : `Section ${item.section}`}</div></td>
